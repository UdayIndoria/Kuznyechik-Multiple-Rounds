...
class Experiment():
    def __init__(self):
        print("Enter which round you want to capture [1-10]:")
        round = int(input())
            ...
            with open(key_files, "rb") as key_file:
                while(True):
                    ...
                    if not len_key:
                        ...
                        zero=0
                        key += zero.to_bytes(32-len_key, 'big') 
                    ...
                    for plaintext_files in [path_plaintext+f for f in os.listdir(path_plaintext) if os.path.isfile(os.path.join(path_plaintext,f))]:
                        ...
                        with open(plaintext_files, "rb") as plaintext_file:
                            while(True):
                                ...
                                if y==1:
                                    starttime = time.time()
                                    ciphertext = kuznyechik_encrypt(plaintext, key, round)
                                    encryption_time += time.time()-starttime
                                    no_of_encryptions += 1
                                else:
                                    ciphertext=random.randint(0, 18446744073709551615)
                                x=str(key)+","+str(ciphertext)+","+str(plaintext)
                                for i in range(128):
